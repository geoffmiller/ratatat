...
function IncrementalRendering() {
  ...
  const serviceCount = Math.min(Math.max(Math.floor(availableLines * 0.7), 5), rows.length)
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      setSelectedIndex((previousIndex) => (previousIndex === 0 ? serviceCount - 1 : previousIndex - 1))
    }

    if (key.downArrow) {
      setSelectedIndex((previousIndex) => (previousIndex === serviceCount - 1 ? 0 : previousIndex + 1))
    }
    ...
    <Box flexDirection="column" height="100%">
      <Box borderStyle="round" borderColor="cyan" paddingX={2} paddingY={1}>
        <Box flexDirection="column">
          ...
          <Text>
            Time: <Text color="green">{timestamp}</Text> • Updates: <Text color="yellow">{counter}</Text> • Random:{' '}
