...
function generateItems(filter: string): string[] {
  ...
  return allItems.filter((item) => item.toLowerCase().includes(filter.toLowerCase())).slice(0, 10)
}
...
function SearchApp() {
  ...
  const filteredItems = useMemo(() => generateItems(deferredQuery), [deferredQuery])
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text dimColor>(Type to search - input stays responsive while list updates)</Text>
      <Box marginTop={1} />
      ...
      <Box marginTop={1} flexDirection="column">
        <Text bold>Results {deferredQuery ? `for "${deferredQuery}"` : '(showing first 10)'}:</Text>
