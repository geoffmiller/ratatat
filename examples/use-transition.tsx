...
import { render, Box, Text, useInput } from '../dist/index.js';
...
function SearchApp() {
    ...
    return (
        <Box flexDirection="column" padding={1}>
            <Text>Search: {query}{isPending ? <Text color="yellow"> (filtering...)</Text> : null}</Text>
            <Box flexDirection="column" marginTop={1}>
                {items.map((item, i) => <Text key={i}>{item}</Text>)}
            </Box>
            <Box marginTop={1}><Text dim>Type to filter. Press Ctrl+C to exit.</Text></Box>
        </Box>
    );
}
