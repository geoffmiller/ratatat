...
const tmpDir = path.join(root, 'sea', '.tmp', 'stress-test-macos-arm64')
const runtimeDir = path.join(tmpDir, 'runtime')
const outputBinary = path.join(outDir, 'ratatat-stress-test')
const checksumFile = path.join(outDir, 'SHA256SUMS-stress-test.txt')
...
// 1) Bundle stress-test into one CJS file for SEA runtime.
await build({
  entryPoints: [path.join(root, 'examples', 'stress-test.tsx')],
